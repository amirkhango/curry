...
def split_to_seq(X, length=3,nb_flow=2):
    w = X.shape[-1]
    h = X.shape[-2]
    X=X.reshape(-1,length, nb_flow, h, w)
    print('Convert for LSTM input shape:', X.shape)
    return X
...
def load_threeway_data(T=24, nb_flow=2, len_closeness=None, len_period=None, len_trend=None, len_test=None, 
    ...
    for l, X_ in zip([len_closeness, len_period, len_trend], [XC_train, XP_train, XT_train]):
        if l > 0:
            #X_train.append(X_)
            X_train.append(split_to_seq(X_, length=l, nb_flow=2))
